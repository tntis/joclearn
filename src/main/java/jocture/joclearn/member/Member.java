...
public class Member {
    ...
    /**
     * - 닉네임 규칙과 이메일 규칙을 다른 곳에서도 사용해야 한다면 어떻게 해야할까? 중복 로직?
     * - 닉네임, 이메일처럼 자체 검증 로직이 필요한 필드가 많아진다면? Member 객체의 책임이 크고 무거워진다.
     * - 부가 로직 때문에 도메인 객체인 Member의 핵심 로직이 잘 보이지 않을 수 있다.
    ...
    private Member(String nickname, String email, String passwordHash, MemberStatus status) {
        ...
        validateEmaill(email);
        this.email = email;
        this.passwordHash = passwordHash;
        this.status = status;
    }
    // validateXXX() - 유효성 검증 실패 시, Exception 발생; Return 타입은 void
    // checkXXX() - validateXXX() 방식일 수도 있고, 결과를 Return 할 수도 있음
    ...
    private void validateEmaill(String email) {
